...
Out-of-the-box Go gives us 2 way to initialize structs - struct literals and the `new` build-in function.
Let’s see how these look like for a simple struct named `Person`:
...
But both of these options contain a big trade-off - now all the fields of the person can be view and changed by anyone.
...
Another, even bigger issue that than is backwards-compatibility.
...
The most clear way to communicate this to the consumers is via doc strings, which is not as obvious as just looking at the method signature (because who reads the docs, right).

A possible mitigation to this problem is if we define the required field in `PersonOptions` as value types and the optional ones as refs.
For example:
...
It is also a bit more clear what is required and what not.
...
Then, the implementation of the method will run all the function one by one on an instance of the struct which will be returned by the constructor.
...
That is why people came up with more sophisticated solutions (based on the build-in ones).

## Takeaways

So what is the best option?
Well, there isn’t one.
All described approaches have they pros and cons.
...
If you are writing a library which will be used by hundreds of other projects, backwards-compatible changes will not be appreciated by your consumers, so you need to choose the more flexible options.
...
If you got to the end of the article, I just want you to know that this I recently started this blog,
and this is the first article in it.
It it was useful for you, you learned something new, or you think it can be improved feel free to ping me on [Twitter](https:/twitter.com/a_sankov), [LinkedIn](https://linkedin.com/in/asankov) or [email](mailto:asankov96+initgo@gmail.com).
